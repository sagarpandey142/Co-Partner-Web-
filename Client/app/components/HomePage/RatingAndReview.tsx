...
import { GetUserDetail } from '../../Services/ProfileHanlder';
...
const ReviewSidebarModal: React.FC<ReviewSidebarModalProps> = ({ setreviewModal }) => {
  const{user}=useUser();
  const[data,setdata]=useState([]);
  const { register, formState: { errors }, getValues, handleSubmit, setValue } = useForm<FormData>();
  useEffect(() => {
    setValue('userExperience', '');
    setValue('User_Rating', 0);
  }, [setValue]);

  const ratingChanged = (data: number) => {
